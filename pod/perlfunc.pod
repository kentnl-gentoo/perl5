...
If DISCIPLINE is omitted or specified as C<:raw> the filehandle is
...
Unicode characters).  Note that as despite what may be implied in
I<"Programming Perl"> (the Camel) or elsewhere C<:raw> is I<not> the
simply inverse of C<:crlf>-- other disciplines which would affect
binary nature of the stream are I<also> disabled. See L<PerlIO>,
L<perlrun> and the discussion about the PERLIO environment variable.
