...
unless the command does not exist (in which case it returns FALSE)
and is executed directly instead of via your system's command shell
(see below).  Use system() instead of exec() if you want it to return.
