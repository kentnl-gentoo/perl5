...
Note that I<currently> fprintf is C<#define'd> to PerlIO_printf,
and stderr to PerlIO_stderr(). This is because 

   fprintf(stderr,...)

is so common in code (including auto-generated code).
So be I<very> carefull with fprintf() as I<only> C<stderr> is auto-mapped.

=item B<PerlIO_stdoutf>

This is printf() equivalent. printf is #defined to this function,
so it is (currently) legal to use printf(...) in perl sources.
